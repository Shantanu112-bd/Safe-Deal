  ...
  return (
    <div className="bg-white text-slate-900">
        ...
        {mobileOpen && (
          <div className="border-t border-slate-200 bg-white px-4 py-4 md:hidden">
            ...
            <div className="mt-4 grid gap-2">
              <GradientButton className="min-w-0 w-full rounded-lg py-2 text-sm">
                Connect Wallet
              </GradientButton>
            </div>
          </div>
      ...
      <main>
        ...
        >
          {/* Right column — Animated Secure Deal Flow */}
          <div className="rounded-3xl border border-white/10 bg-white/5 backdrop-blur p-6 shadow-xl shadow-black/20">
            <p className="mb-6 text-sm font-semibold text-slate-400">Animated Secure Deal Flow</p>
            <div className="space-y-4">
              {[
                { icon: Wallet,       label: "Buyer",            tone: "bg-blue-900/40 text-blue-300 border-blue-700/40" },
                { icon: Lock,         label: "Locks Payment",    tone: "bg-emerald-900/40 text-emerald-300 border-emerald-700/40" },
                { icon: Shield,       label: "SafeDeal Vault",   tone: "bg-white/10 text-white border-white/20" },
                { icon: Send,         label: "Seller Ships",     tone: "bg-amber-900/40 text-amber-300 border-amber-700/40" },
                { icon: CheckCircle2, label: "Buyer Confirms",   tone: "bg-emerald-900/40 text-emerald-300 border-emerald-700/40" },
                { icon: HandCoins,    label: "Money Released",   tone: "bg-white/10 text-white border-white/20" },
              ].map((item, index) => {
              ...
              {badges.map((badge, index) => (
                <article key={badge} className="rounded-2xl border border-slate-200 bg-slate-50 p-5 text-center">
                  ...
                  {index < badges.length - 1 && (
                    <p className="mt-1 text-sm text-slate-500">Next badge unlocks after more deals</p>
                  )}
                </article>
              ))}
            </div>
        ...
        <section id="pricing" className="border-y border-slate-200 bg-[#0f172a] py-12 text-white">
          <div className="relative mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8">
            ...
            <SparklesCore
              ...
              speed={1}
            />
            <div className="relative z-10 grid gap-6 text-center sm:grid-cols-2 lg:grid-cols-4">
              {stats.map((stat) => (
        ...
        <section className="bg-gradient-to-b from-white to-slate-50 py-20">
          <div className="mx-auto w-full max-w-4xl rounded-3xl border border-slate-200 bg-white px-6 py-14 text-center shadow-lg shadow-slate-900/5 sm:px-10">
            ...
            <div className="mt-8 flex flex-wrap items-center justify-center gap-3">
              ...
              <GradientButton variant="variant" onClick={() => router.push("/how-it-works")}>
                Get Started
              </GradientButton>
            </div>
          </div>
